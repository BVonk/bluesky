...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_size, action_size, max_aircraft, BATCH_SIZE, TAU, LEARNING_RATE):
        ...
        self.model, self.weights, self.actions,  self.state = BiCNet.build_actor(max_aircraft, state_size, action_size, 128, 128, 'actor')
        # print(self.model.summary())
        self.target_model, self.target_weights, self.target_actions, self.target_state = BiCNet.build_actor(max_aircraft, state_size, action_size, 128, 128, 'actor_target')
...
class ActorNetwork_shared_obs(object):
    ...
    def __init__(self, sess, state_size, action_size, max_aircraft, BATCH_SIZE, TAU, LEARNING_RATE):
        ...
        #Now create the model
        self.model, self.weights, self.actions,  self.state, self.shared_state = BiCNet.build_actor_shared_obs(max_aircraft, state_size[0], state_size[1], action_size, 128, 128, 'actor')
        # print(self.model.summary())
        self.target_model, self.target_weights, self.target_actions, self.target_state, self.target_shared_state = BiCNet.build_actor_shared_obs(max_aircraft, state_size[0], state_size[1], action_size, 128, 128, 'actor_target')
        self.action_gradient = tf.placeholder(tf.float32,[None, None, action_size])
        # Negative action gradients are used for gradient ascent.
